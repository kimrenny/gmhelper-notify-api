...
func setupAutomationTest() (*AutomationHandler, *handlerMockAutomationRepo, *handlerMockTemplateRepo) {
	...
	return handler, autoRepo, tplRepo
}

func TestAutomationHandler_List(t *testing.T) {
	handler, autoRepo, _ := setupAutomationTest()

	now := time.Now().UTC()
	autoRepo.rules["r1"] = &domain.AutomationRule{
		ID:         "r1",
		Name:       "Rule 1",
...
func TestAutomationHandler_GetByID(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/api/v1/automation/rules/r1", nil)
	...
	if res.ID != "r1" || res.Name != "Welcome Rule" || !res.Enabled {
	...
	reqNotFound := httptest.NewRequest(http.MethodGet, "/api/v1/automation/rules/non-existent", nil)
	reqNotFound.SetPathValue("id", "non-existent")
...
func TestAutomationHandler_Update(t *testing.T) {
	...
	now := time.Now().UTC()
	autoRepo.rules["r-edit"] = &domain.AutomationRule{
		ID:         "r-edit",
		Name:       "Original Name",
		TemplateID: "tpl-valid-1",
		Enabled:    true,
	...
	req := httptest.NewRequest(http.MethodPut, "/api/v1/automation/rules/r-edit", bytes.NewReader(body))
	req.SetPathValue("id", "r-edit")
	rec := httptest.NewRecorder()
	...
	reqMalformed := httptest.NewRequest(http.MethodPut, "/api/v1/automation/rules/r-edit", bytes.NewReader([]byte("{invalid")))
	reqMalformed.SetPathValue("id", "r-edit")
	recMalformed := httptest.NewRecorder()
	...
	reqInv := httptest.NewRequest(http.MethodPut, "/api/v1/automation/rules/r-edit", bytes.NewReader(bodyInv))
	reqInv.SetPathValue("id", "r-edit")
	recInv := httptest.NewRecorder()
	...
	reqBadTpl := httptest.NewRequest(http.MethodPut, "/api/v1/automation/rules/r-edit", bytes.NewReader(bodyBadTpl))
	reqBadTpl.SetPathValue("id", "r-edit")
	...
	reqNotFound := httptest.NewRequest(http.MethodPut, "/api/v1/automation/rules/r-404", bytes.NewReader(body))
	reqNotFound.SetPathValue("id", "r-404")
	recNotFound := httptest.NewRecorder()
...
func TestAutomationHandler_Delete(t *testing.T) {
	...
	autoRepo.rules["r-del"] = &domain.AutomationRule{
		ID:         "r-del",
		Name:       "Rule To Delete",
		TemplateID: "tpl-valid-1",
		Enabled:    true,
	...
	req := httptest.NewRequest(http.MethodDelete, "/api/v1/automation/rules/r-del", nil)
	req.SetPathValue("id", "r-del")
	rec := httptest.NewRecorder()
	...
	if _, exists := autoRepo.rules["r-del"]; exists {
	...
	// 3. Not Found -> 404 Not Found
	reqNotFound := httptest.NewRequest(http.MethodDelete, "/api/v1/automation/rules/r-del", nil)
	reqNotFound.SetPathValue("id", "r-del")
	recNotFound := httptest.NewRecorder()
	...
	// 4. Service error -> 500
	autoRepo.rules["r-err"] = &domain.AutomationRule{ID: "r-err"}
	autoRepo.deleteErr = errors.New("db error")
	reqErr := httptest.NewRequest(http.MethodDelete, "/api/v1/automation/rules/r-err", nil)
	reqErr.SetPathValue("id", "r-err")
	recErr := httptest.NewRecorder()
